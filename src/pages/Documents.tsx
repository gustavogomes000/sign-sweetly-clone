...
export default function Documents() {
  ...
  return (
    <>
      <AppHeader
        title="Documentos"
        subtitle={`${filteredDocs.length} de ${mockDocuments.length} documentos`}
      />
          ...
          {selectedDocs.length > 0 && (
            <div className="flex items-center gap-2 p-3 bg-primary/5 rounded-lg border border-primary/20 animate-fade-in">
              ...
              <div className="flex items-center gap-1 ml-auto">
                ...
                <Button variant="outline" size="sm"><Download className="w-3 h-3 mr-1" />Baixar</Button>
                <Button variant="outline" size="sm" className="text-destructive"><Trash2 className="w-3 h-3 mr-1" />Cancelar</Button>
              </div>
            </div>
          )}
        </div>
                ...
                >
                  ...
                  <Link to={`/documents/${doc.id}`} className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        {doc.folder && (
                          <span className="text-[10px] px-1.5 py-0.5 bg-secondary rounded text-muted-foreground">{doc.folder}</span>
                        )}
                        {doc.tags?.map((tag) => (
                          <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-primary/10 text-primary rounded">{tag}</span>
                        ))}
                        {doc.envelope && (
                          <span className="text-[10px] text-muted-foreground">{doc.envelope}</span>
                        )}
                      </div>
                    </div>
                  </Link>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem><Send className="w-4 h-4 mr-2" />Reenviar</DropdownMenuItem>
                      ...
                      <DropdownMenuItem><Tag className="w-4 h-4 mr-2" />Editar tags</DropdownMenuItem>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem className="text-destructive"><Trash2 className="w-4 h-4 mr-2" />Cancelar</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
              ))}
                ...
                <Card className="hover:shadow-md transition-all hover:border-primary/30 h-full">
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap mb-3">
                      {doc.folder && <span className="text-[10px] px-1.5 py-0.5 bg-secondary rounded text-muted-foreground">{doc.folder}</span>}
                      {doc.tags?.map((tag) => (
                        <span key={tag} className="text-[10px] px-1.5 py-0.5 bg-primary/10 text-primary rounded">{tag}</span>
                      ))}
                    </div>
