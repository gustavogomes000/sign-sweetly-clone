...
export default function Documents() {
  ...
  const filteredDocs = documents
    .filter((doc) => {
      const matchesSearch = doc.nome.toLowerCase().includes(search.toLowerCase()) ||
        doc.signers.some(s => s.nome.toLowerCase().includes(search.toLowerCase()));
  ...
  const toggleSelect = (id: string) => setSelectedDocs((prev) => prev.includes(id) ? prev.filter((d) => d !== id) : [...prev, id]);
  const toggleSelectAll = () => { if (selectedDocs.length === filteredDocs.length) setSelectedDocs([]); else setSelectedDocs(filteredDocs.map((d) => d.id)); };
  ...
  return (
    <>
      <AppHeader title="Documentos" subtitle={`${filteredDocs.length} de ${documents.length} documentos`} />
      <div className="flex-1 overflow-auto p-6 space-y-4">
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between gap-3">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <Input placeholder="Buscar por nome ou signatário..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 h-9 w-72 bg-secondary/50 border-border/50" />
            </div>
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="icon" className="h-9 w-9" onClick={() => setSortBy(sortBy === 'date' ? 'name' : 'date')}><ArrowUpDown className="w-4 h-4" /></Button>
              <div className="flex border border-border/50 rounded-lg overflow-hidden">
                <button onClick={() => setViewMode('list')} className={cn('p-2 transition-colors', viewMode === 'list' ? 'bg-primary/10 text-primary' : 'hover:bg-secondary/50')}><List className="w-4 h-4" /></button>
                <button onClick={() => setViewMode('grid')} className={cn('p-2 transition-colors', viewMode === 'grid' ? 'bg-primary/10 text-primary' : 'hover:bg-secondary/50')}><Grid3X3 className="w-4 h-4" /></button>
              </div>
            ...
            <div className="divide-y divide-border/30">
              ...
              {filteredDocs.map((doc, i) => (
                <motion.div key={doc.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                  className={cn('flex items-center gap-3 px-4 py-3 hover:bg-primary/5 transition-colors group', selectedDocs.includes(doc.id) && 'bg-primary/5')}>
                        ...
                        <div className="flex -space-x-1.5">
                          {doc.signers.slice(0, 3).map((s) => (
                            <div key={s.id} className={cn('w-6 h-6 rounded-full border-2 border-card flex items-center justify-center text-[9px] font-bold',
                              s.status === 'signed' ? 'bg-success/20 text-success' : s.status === 'refused' ? 'bg-destructive/20 text-destructive' : 'bg-warning/20 text-warning')}>
                              {s.nome.charAt(0)}
                            </div>
                          ))}
                        </div>
