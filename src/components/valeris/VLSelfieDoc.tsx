...
export function VLSelfieDoc({ apiKey, aoCompletar, onError }: VLSelfieDocProps) {
  const [loading, setLoading] = useState(false);

  const handleAction = async (data: any) => {
    try {
      setLoading(true);
      const endpoint = '/v2/selfie-documento/capturar';
  ...
  return (
    <div className="p-4 border border-border rounded-lg bg-card text-foreground">
      <h3 className="text-lg font-bold mb-4">VLSelfieDoc</h3>
