...
export function VLSelfieDoc({ apiKey, signatoryId, documentId, aoCompletar, onError }: VLSelfieDocProps) {
  ...
  const confirm = async () => {
    ...
    try {
      if (apiKey) {
        const res = await fetch('https://api.valeris.com.br/v2/selfie-documento/capturar', {
          method: 'POST',
          headers: { 
        ...
        console.warn('[VLSelfieDoc] API key not configured, completing locally');
